...
export function runGate(args = process.argv.slice(2)) {
  ...
  if (mode === 'live') {
    ...
    console.log('Public beta full release gate passed: unit/API tests, product-document validation, UX contracts, TypeScript, production build, deterministic E2E, accessibility, and live provider canaries are clean.');
  } else {
    console.log('Public beta deterministic gate passed: unit/API tests, product-document validation, UX contracts, TypeScript, production build, deterministic E2E, and accessibility are clean.');
  }
}
