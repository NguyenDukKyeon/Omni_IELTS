...
function declaredLengthIsWithinLimit(request: Request): boolean {
  const raw = request.get('content-length');
  if (raw === undefined) return true;
  if (!/^\d+$/.test(raw.trim())) return false;
  const length = Number(raw);
  return Number.isSafeInteger(length) && length <= SOURCE_IMPORT_MAX_JSON_BODY_BYTES;
}
...
export function createJsonParserExcludingSourceImport(genericParser: RequestHandler): RequestHandler {
  return (request, response, next) => {
    if (isSourceImportRoute(request)) return next();
...
export function createSourceImportAdmissionGate(options: SourceImportAdmissionOptions): RequestHandler {
  return async (request, response, next) => {
    ...
    if (!contentTypeIsJson(request) || !declaredLengthIsWithinLimit(request)) {
