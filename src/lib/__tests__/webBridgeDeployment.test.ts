...
describe('private Gemini Web bridge deployment', () => {
  ...
  it('requires an authenticated cookie file and never enables anonymous mode', () => {
    ...
    expect(entrypoint).toContain('"gemini-pro"');
  ...
  it('requires live generation evidence from the authenticated Pro model', () => {
    ...
    expect(canary).toContain("payload.model !== 'gemini-3.1-pro'");
    expect(canary).toContain("payload.resolvedModel !== 'gemini-pro'");
  ...
  it('does not send grounded forecast synthesis to Gemini Web Pro', () => {
    const server = readFileSync(resolve(root, 'server.ts'), 'utf8');
  ...
  it('cancels stalled Pro generations before they can retain the single-flight lock', () => {
    const compose = readFileSync(resolve(root, 'compose.media.yml'), 'utf8');
    const entrypoint = readFileSync(resolve(root, 'infra/gemini-web2api/entrypoint.py'), 'utf8');
    const canary = readFileSync(resolve(root, 'scripts/web-bridge-live-canary.mjs'), 'utf8');
  ...
  it('bounds the complete official fallback chain after a deep Pro failure', () => {
    const server = readFileSync(resolve(root, 'server.ts'), 'utf8');
