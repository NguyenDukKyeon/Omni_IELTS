...
export const UX_FLOW_CONTRACTS: UxFlowContract[] = [
  ...
  {
    id: 'sources.chat.send', module: 'sources', owner: 'SourceGroundedChat',
    precondition: 'At least one usable source version is selected', trigger: 'Learner asks a question about the selected sources',
    expectedTransition: 'idle -> loading -> cited answer|typed refusal', sideEffect: 'POST /api/sources/grounded-chat with selected IDs and optional span only',
    errorStates: ['auth_required', 'quota_exceeded', 'unavailable', 'unsupported_by_sources'], evidence: ['src/lib/__tests__/sourcesGroundedChatUi.test.ts'],
  },
  {
    id: 'sources.chat.web-research', module: 'sources', owner: 'SourceGroundedChat',
    precondition: 'A learner question is present', trigger: 'Learner explicitly requests web evidence',
    expectedTransition: 'idle -> loading -> web citations|typed failure', sideEffect: 'POST /api/sources/web-research only after explicit activation',
    errorStates: ['auth_required', 'quota_exceeded', 'unavailable'], evidence: ['src/lib/__tests__/sourcesGroundedChatUi.test.ts'],
  },
  {
    id: 'sources.chat.citation-open', module: 'sources', owner: 'CitationDrawer',
    precondition: 'A cited server response is visible', trigger: 'Learner opens a citation chip',
    expectedTransition: 'citation chip -> cited block drawer', sideEffect: 'Update local citation presentation state',
    errorStates: [], evidence: ['src/lib/__tests__/sourcesGroundedChatUi.test.ts'],
  },
  {
    id: 'sources.reader.select-span', module: 'sources', owner: 'SourceReader',
  ...
  },
  {
    ...
    precondition: 'One destination and a usable source version or span are selected', trigger: 'Learner generates one artifact draft',
    expectedTransition: 'picker -> processing -> validating -> ready|needs_review|typed failure', sideEffect: 'Persist only SourceArtifactJob state; destination owner persists later',
    errorStates: ['auth_required', 'quota_exceeded', 'unavailable', 'validation_failed'], evidence: ['src/lib/__tests__/sourcesArtifactStudioUi.test.ts'],
  },
  {
    id: 'sources.artifact.open', module: 'sources', owner: 'ArtifactDraftPreview',
    precondition: 'A genuine validated draft has a navigable handoff', trigger: 'Learner opens the artifact',
    expectedTransition: 'ready draft -> destination module', sideEffect: 'Navigate only after activation; destination owner receives the draft reference',
    errorStates: ['destination_unavailable'], evidence: ['src/lib/__tests__/sourcesArtifactStudioUi.test.ts'],
  },
  {
    id: 'sources.artifact.create-another', module: 'sources', owner: 'ArtifactStudioModal',
    precondition: 'A ready draft preview is visible', trigger: 'Learner creates another output',
    expectedTransition: 'ready draft -> destination picker', sideEffect: 'Reset destination state only; preserve source selection',
    errorStates: [], evidence: ['src/lib/__tests__/sourcesArtifactStudioUi.test.ts'],
  },
  {
    id: 'sources.view.tabs', module: 'sources', owner: 'SourcesView',
    precondition: 'The flag-on Sources workspace is visible', trigger: 'Learner switches between Library, Reader & Chat, and Create',
    expectedTransition: 'active workspace pane -> requested workspace pane', sideEffect: 'Update local Sources view tab state',
    errorStates: [], evidence: ['src/lib/__tests__/sourcesViewFacade.test.ts'],
  },
  {
...
export const UX_CONTROL_CONTRACTS: UxControlContract[] = [
  ...
  sourcesControl('sources.chat.citation-close', 'sources.chat.citation-open', 'CitationDrawer', 'Close the citation drawer', 'citation drawer open', 'citation drawer closed'),
  sourcesControl('sources.chat.composer', 'sources.chat.send', 'SourceGroundedChat', 'Submit the grounded chat composer', 'question entered', 'answer or typed refusal'),
  sourcesControl('sources.chat.question-input', 'sources.chat.send', 'SourceGroundedChat', 'Edit the grounded chat question', 'chat composer visible', 'question edited'),
  sourcesControl('sources.chat.retry', 'sources.chat.send', 'SourceGroundedChat', 'Retry a grounded question', 'retryable chat error visible', 'chat loading'),
  sourcesControl('sources.chat.research-retry', 'sources.chat.web-research', 'SourceGroundedChat', 'Retry explicit web research', 'retryable web research error visible', 'web research loading'),
  sourcesControl('sources.chat.web-result', 'sources.chat.web-research', 'SourceGroundedChat', 'Open a returned web citation', 'web citations visible', 'source opens in a new tab'),
  ...
  sourcesControl('sources.artifact.custom-instruction', 'sources.artifact.generate', 'ArtifactStudioModal', 'Add a bounded custom instruction', 'Artifact Studio open', 'instruction edited'),
  sourcesControl('sources.artifact.open', 'sources.artifact.open', 'ArtifactDraftPreview', 'Open the validated destination artifact', 'navigable ready draft visible', 'destination module opened'),
  sourcesControl('sources.artifact.create-another', 'sources.artifact.create-another', 'ArtifactStudioModal', 'Create another output from the same source', 'ready draft preview visible', 'destination picker visible'),
  sourcesControl('sources.view.tab-library', 'sources.view.tabs', 'SourcesView', 'Open the Sources Library pane', 'Sources workspace visible', 'Library pane visible'),
  sourcesControl('sources.view.tab-reader', 'sources.view.tabs', 'SourcesView', 'Open the Reader and Chat pane', 'Sources workspace visible', 'Reader and Chat pane visible'),
  sourcesControl('sources.view.tab-create', 'sources.view.tabs', 'SourcesView', 'Open the Create pane', 'Sources workspace visible', 'Create pane visible'),
  sourcesControl('sources.view.open-create', 'sources.artifact.open-modal', 'SourcesView', 'Open Artifact Studio for the active source', 'active ready source visible', 'Artifact Studio open'),
  ...
  sourcesControl('sources.import.url', 'sources.import.submit', 'SourceImportPanel', 'Enter an article URL', 'URL import selected', 'URL edited'),
  sourcesControl('sources.import.pdf', 'sources.import.submit', 'SourceImportPanel', 'Choose a PDF payload', 'PDF import selected', 'PDF file selected'),
  sourcesControl('sources.import.docx', 'sources.import.submit', 'SourceImportPanel', 'Choose a DOCX payload', 'DOCX import selected', 'DOCX file selected'),
  sourcesControl('sources.import.vtt', 'sources.import.submit', 'SourceImportPanel', 'Paste VTT or SRT captions', 'caption import selected', 'caption content edited'),
